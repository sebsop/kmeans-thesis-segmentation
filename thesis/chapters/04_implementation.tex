...
A thread block size of $16 \times 16$ ($256$ threads per block) is selected. In the NVIDIA Ampere architecture, this block size maximizes Streaming Multiprocessor (SM) occupancy \cite{CudaGuide}. It provides a balanced distribution of registers per thread, allowing the hardware scheduler to hide memory latency by keeping warps highly active without running out of registers \cite{CudaGuide, cook2012cuda}.
...
The 5D features are stored in an Array of Structures (AoS) memory layout: $[B_0, G_0, R_0, x_0, y_0, B_1, G_1, R_1, x_1, y_1...]$. While CUDA often favors a Structure of Arrays (SoA) for coalesced access \cite{CudaGuide, cook2012cuda}, the AoS layout was chosen to prioritize thread-level cache line utilization. 
...
Host-side buffers are allocated using page-locked (pinned) memory via \texttt{cudaMallocHost}. This prevents the operating system from swapping these buffers to disk, allowing the GPU to use DMA to copy processed frames directly to system RAM without involving the CPU \cite{CudaGuide}. 
...
\label{tab:hyperparameters}
    ...
    \hline
        ...
        Clustering & \texttt{FEATURE\_DIMS} & 5 & Defines the manifold as $[B, G, R, x, y]$. \\ \hline
         & \texttt{K\_MIN - K\_MAX} & 2 - 40 & Constrains model complexity for real-time stability. \\ \hline
...
To evaluate the segmentation quality, the system runs three quality metrics in the background: WCSS \cite{lloyd1982least,krzanowski1988criterion}, the Davies-Bouldin Index \cite{davies1979cluster,xiao2017davies}, and the Silhouette Coefficient \cite{rousseeuw1987silhouettes, shutaywi2021silhouette}. 

To prevent these calculations from blocking the real-time pipeline, all three metrics are implemented within the \texttt{BenchmarkRunner} class and executed on a dedicated background thread \cite{williams2019c++}. The optimized CPU class functions in \texttt{src/clustering/metrics.cpp} are used to process the telemetry data. 

For the Silhouette Coefficient, calculating the score exactly is computationally impractical. As derived in Section \ref{subsect:silhouette_complexity}, a downscaled frame of $320 \times 240$ pixels requires approximately $2.95$ billion distance calculations, which would briefly freeze the UI loop. A Monte Carlo approximation is implemented directly within the background metrics thread, sampling a representative subset of $M=2,000$ points \cite{shutaywi2021silhouette}. This allows for calculating and displaying the statistical Davies-Bouldin index and Silhouette score on the dashboard in under a millisecond, providing real-time feedback without lagging the video stream.
