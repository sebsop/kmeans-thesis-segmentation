...
To execute 5D clustering in parallel on the GPU, the 2D image coordinates must be mapped to CUDA's thread and block grids \cite{CudaGuide}. This mapping allows the hardware scheduler to distribute pixel computations across Streaming Multiprocessors to maximize occupancy.
...
To achieve high-throughput clustering, the system minimizes global VRAM latency through a two-tier synchronization strategy. This approach aggregates pixel updates locally in fast on-chip shared memory before committing them to global GPU buffers.
...
To maintain a high-speed video feed, the visualization pipeline decouples GPU calculations from CPU rendering. This allows the compute engine to run asynchronously in CUDA streams while the host thread updates the OpenGL display.
