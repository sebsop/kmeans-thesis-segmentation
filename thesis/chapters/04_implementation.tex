...
Sustained throughput is achieved by tuning the hardware and operating system, which ensures deterministic latency and prevents frame-time variance. On the software side, the Windows scheduler was optimized via the registry (\texttt{Win32PrioritySeparation} set to \texttt{0x26}) to enable short, variable clock quantums, prioritizing foreground CPU slices for the camera input/output (I/O) and event loops \cite{microsoft_priority_separation_official, intel_thermal_ultra_gen}. On the hardware side, rather than using max-frequency profiles that induce thermal throttling, a high performance profile was adopted to manage the thermal design power (TDP) between the CPU and GPU \cite{intel_thermal_ultra_gen}, providing the stable thermal headroom necessary for continuous high-speed execution.

\subsection{Tooling and Code Quality}
The codebase is managed using the CMake build system \cite{cmake_official}, which handles the integration of C++20, OpenCV, GLFW, and the CUDA compiler (NVCC). Automated formatting via \texttt{clang-format} and static analysis via \texttt{clang-tidy} maintain clean and consistent code \cite{clang_tools}. This setup helps prevent memory leaks and common C++ errors, ensuring benchmark results reflect the actual efficiency of the algorithms rather than implementation bugs.

\subsection{Threading Model and GPU Grid Layout}
Parallel execution of 5D clustering on the GPU requires mapping 2D image coordinates to CUDA's thread and block grids \cite{CudaGuide}. This mapping allows the hardware scheduler to distribute pixel computations across Streaming Multiprocessors to maximize occupancy.
