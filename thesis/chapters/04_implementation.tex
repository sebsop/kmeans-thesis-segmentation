...
{\itshape Implementing a real-time segmentation pipeline capable of processing over $120\text{ FPS}$ requires optimization tailored to low-level GPU hardware. The concrete C++ and CUDA implementation details are presented, focusing on how thread grids are configured, on-chip shared memory is utilized, and hardware-accelerated math intrinsics are applied. Additionally, the orchestration of CPU-GPU memory transfers through asynchronous streams is described to maximize pipeline throughput.}
