...
{\itshape Implementing a real-time segmentation pipeline capable of processing over $120\text{ FPS}$ requires optimization tailored to low-level GPU hardware. We present the concrete C++ and CUDA implementation details, focusing on how thread grids are configured, on-chip shared memory is utilized, and hardware-accelerated math intrinsics are applied. Additionally, we describe the orchestration of CPU-GPU memory transfers through asynchronous streams to maximize pipeline throughput.}
...
The 5D features are stored in an AoS memory layout: $[B_0, G_0, R_0, x_0, y_0, B_1, G_1, R_1, x_1, y_1...]$. While CUDA often favors a Structure of Arrays (SoA) for coalesced access \cite{CudaGuide, cook2012cuda}, the AoS layout was chosen to prioritize thread-level cache line utilization. 
