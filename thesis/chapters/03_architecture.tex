...
\section{Architectural Overview and Design Patterns}
The segmentation framework is designed as a high-throughput, asynchronous pipeline that processes live camera feeds with minimal latency. Decoupling the GUI from the computational backend keeps the user interface responsive during heavy GPU calculations. This separation is implemented using a multi-threaded architecture with a dedicated worker thread, as shown in Figure~\ref{fig:system_architecture}.
...
\begin{tikzpicture}[
    ...
    % ==========================================
    \draw[dataLine, color=orange!80!black, dashed] (GLFW.north) -- (CfgL.south)
    ...
    \draw[dataLine, color=purple!80] (CfgL.north) -- (CV.south)
        node[text width=2.8cm, pos=0.6, left, xshift=3.2cm, font=\scriptsize\sf] {2. Reads Hyperparameters};
...
\subsection{System Pipeline and Concurrency Model}
...
Real-time performance is achieved by running the application on two main threads that communicate asynchronously:
\begin{itemize}
    \item \begin{minipage}[t]{\linewidth}
    \textbf{UI Thread (Main):} Handles the application lifecycle, processes window events, and renders the immediate-mode GUI using Dear ImGui \cite{ImGui}. It retrieves fully processed segmented frames from shared buffers using a mutex-protected synchronization lock \cite{williams2019c++}.
    \end{minipage}
    \item \begin{minipage}[t]{\linewidth}
    \textbf{Clustering Worker Thread:} Managed by the core \texttt{Application} class, this thread runs the main clustering loop. It continuously updates hyperparameters from the UI, executes the active CUDA clustering engine, and commits the resulting segmented frames to shared buffers for UI rendering.
    \end{minipage}
\end{itemize}
...
Coupling between the user interface and high-performance computation is prevented by using specific software design patterns to separate the GUI from the underlying mathematical engines. This structural isolation ensures the system remains modular, extensible, and easy to maintain \cite{gamma1995design}.

\subsection{Design Patterns and Software Architecture}

\paragraph{Creational and Structural Design Patterns}
...
\paragraph{Behavioral and Concurrency Design Patterns}
...
\begin{itemize}
    ...
    \item \textbf{Resource Acquisition Is Initialization (RAII):} The \texttt{CudaAssignment\allowbreak Context} manages the lifetime of all GPU resources. It handles the automatic allocation and deterministic release of device memory (VRAM) and page-locked host memory, preventing memory leaks in high-frequency loops \cite{stroustrup2022tour}.
\end{itemize}
...
\subsection{The 5D Color-Spatial Manifold and Strided Sampling}\label{subsect:normalized_manifold}
...
A spatial scaling weight ($\lambda_{\text{spatial}} = 10^{-6}$) is applied to prevent geometric coordinates from dominating the distance calculations on high-resolution frames. This weight ensures that chromatic similarity drives the segmentation, while spatial information acts as a regularizer to prevent noisy, fragmented pixel regions \cite{shi2000normalized, achanta2012slic}.

Maintaining a stable frame rate requires decoupling the iterative centroid learning phase from the final pixel assignment phase using the parallel strided downsampling theory detailed in Section~\ref{subsect:downsampling_theory} \cite{achanta2012slic}. The preprocessor samples the frame at regular spatial strides $S \in [1, 16]$, reducing the total number of points in the learning feature space by a factor of $S^2$, which substantially lowers computational complexity. Statistically, by the Law of Large Numbers, this uniform subsampling of thousands of pixels remains highly representative of the image's overall color-spatial distribution. This ensures that the estimated centroids approximate the true global cluster centers with high fidelity \cite{mouton2020stride} while the resulting sparse manifold is uploaded to the GPU as a contiguous memory block to enable fast, coalesced memory access \cite{CudaGuide}.
...
\section{Component Specifications and Thread Interoperability}
The C++/CUDA implementation is organized into highly modular classes to isolate parallel mathematical solvers from the dashboard user interface. This separation of concerns ensures that the performance-critical GPU kernel execution remains decoupled from frontend rendering logic.

\subsection{Core Software Component Specifications}

\paragraph{ClusteringManager (The Orchestrator)}
...
\paragraph{CudaAssignmentContext (Resource Container)}
...
\paragraph{Engine Hierarchy and Factory}
...
\paragraph{StridedDataPreprocessor}
...
\subsection{User Interface and Telemetry Dashboard}\label{subsect:ui_telemetry}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/control_panel.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/benchmark_metrics.png}
...
Achieving low latency requires optimizing both the physical transfers across the PCIe bus and the thread synchronization within the application layer. This requires combining asynchronous host-device memory operations with thread-safe data structures.

\subsection{Thread Synchronization and Heterogeneous Communication}
The main bottleneck in heterogeneous computing is the latency of transferring data across the PCIe bus \cite{CudaGuide}. Minimizing this transfer overhead involves using page-locked (pinned) host memory \cite{CudaGuide, cook2012cuda}. Pinned memory enables the GPU to use DMA to copy features directly from host memory without CPU intervention \cite{CudaGuide}. 
...
The \texttt{ClusteringManager} executes on a dedicated worker thread to keep the GUI running smoothly, preventing heavy GPU computations from blocking the main GLFW render loop \cite{GLFWdocs}. 
