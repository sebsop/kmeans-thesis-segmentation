...
The GUI dashboard is implemented using Dear ImGui \cite{ImGui}, providing a lightweight control layer that runs directly inside the GLFW \cite{GLFWdocs} loop. Decoupling is maintained by the \texttt{UIManager} class, which manages three main components shown in Figure \ref{fig:ui_dashboard}:
\begin{itemize}
    ...
    \item \textbf{BenchmarkOverlayUI:} Renders a full-screen overlay showing the original still frame alongside both segmented engine outputs. It displays comparative telemetry (quality metrics, latencies, and iteration counts) and provides controls to tune parameters ($K$, stride $S$, and seeding strategy), toggle centroids, and trigger reruns on the frozen frame using shared seeds.
\end{itemize}

\begin{figure}[htbp]
    \centering
    %\includegraphics[width=0.9\textwidth]{figures/dashboard_active.png}
    \vspace{0.8em}
    %\includegraphics[width=0.9\textwidth]{figures/dashboard_benchmark.png}
    \caption[Dear ImGui Dashboard and Telemetry Overlay]{The Dear ImGui user interface, displaying (top) the active video feed and control panel, and (bottom) the side-by-side benchmarking overlay.}
